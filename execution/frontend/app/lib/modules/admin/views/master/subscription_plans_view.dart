...
class _SubscriptionPlansViewState extends State<SubscriptionPlansView> {
  ...
  Future<void> _load() async {
    ...
    if (mounted)
      setState(() {
        _plans = (rows as List).cast<Map<String, dynamic>>();
        _loading = false;
      });
  }
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        TextField(
                          ...
                          decoration: InputDecoration(
                            labelText: 'Stripe Price ID (optional)',
                            labelStyle: ETextStyles.inputLabel,
                          ),
                        ),
                        const SizedBox(height: ESpacing.sm),
                      ...
                      onPressed: () async {
                        ...
                        final priceCents =
                            ((double.tryParse(priceCtrl.text.trim()) ?? 0) *
                                ...
                                .round();
                        final features =
                            featuresCtrl.text.trim().isEmpty
                                ? <String>[]
                                : featuresCtrl.text
                                    .split(',')
                                    .map((s) => s.trim())
                        ...
                        await _db.from('subscription_plans').insert({
                          ...
                          'stripe_price_id':
                              stripeCtrl.text.trim().isEmpty
                                  ? null
                                  : stripeCtrl.text.trim(),
            ...
            _loading
                ...
                : ListView.separated(
                  ...
                  itemBuilder: (_, i) {
                    final p = _plans[i];
                    final hasPriceId =
                        (p['stripe_price_id'] as String?)?.isNotEmpty ?? false;
